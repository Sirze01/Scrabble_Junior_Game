 ...
 * @param regularMessage - command responses to show in screen
 * @param coloredMessage - important message that cuts command access
 * @param ableToMove - to update ability to move for moveHandler to react accordingly
 * @return true - valid pass or command (moveHandler may return)
 * @return false - moveHandler must continue the loop
 */
bool Game::processHumanCommand(const std::string& input, std::stringstream& regularMessage, std::stringstream& coloredMessage, bool &ableToMove) {
	std::string playerName = _currentPlayer->getName();
...
/**
 * Generate a bot action (exchange, move or pass) and execute it.
 * @param coloredMessage - important message that cuts command access.
 * @param ableToMove - to update ability to move for moveHandler to react accordingly.
 * @param botName - to output the stream with player's name.
 */
void Game::botMove(std::stringstream& coloredMessage, bool ableToMove, const std::string& botName) {
	for (;;) { //exits when move is valid or exchanged once
		if (ableToMove) {
			...
			move.execute(*_currentPlayer, _board, _pool);
			coloredMessage << botName << " will play " << Board::getPositionString(pos) << SPACE << letter << ".\n";
			break;
		}
			...
			coloredMessage << botName << " exchanged letter " << oldLetter << " from the pool and got letter "
				<< newLetter << ".\n\n";
			ableToMove = _currentPlayer->mayMove(_board);
			showPlayerInfo(true);
		}
		else { //exchange is not possible
			coloredMessage << botName << " cannot move and thus will pass.\n";
			_currentPlayer->forcePass();
			break;
		}
	}
 ...
 * If the player is human, gives access for commands and loops while the entered command is a valid move or pass.
...
void Game::moveHandler(int turnNumber) {
	...
	for (;;) { //breaks when command is a valid move, player passes, or colored message has content
		...
		if (!bot) { //user has access to commands
			...
			passOrMove = processHumanCommand(input, regularMessage, coloredMessage, ableToMove);
		}

		if (!Util::isEmpty(regularMessage)) { //print command response and ask again
			Util::paddingAndTopic(WHITE, true);
			std::cout << regularMessage.str();
		}
		else if (!Util::isEmpty(coloredMessage)) { //cut access to commands and return
		...
		else if (passOrMove) return;
		else continue; //card view command or clear, no message
	}
}
