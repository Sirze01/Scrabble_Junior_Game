...
bool Board::highlightFinishedWord(int color, int vIndex, int hIndex) {
    bool successOnCol = true, successOnLine = true;

    //iterate collumn
    for (int line = vIndex ; line >= 0; line--) {
        if (_letters.at(line).at(hIndex) == ' ') break;
        else if (!_highlights.at(line).at(hIndex)) {
            successOnCol = false; break;
        }
    }
    if (successOnCol) {
        for (int line = vIndex; line >= 0; line--) {
    ...
    //iterate line
    for (int col = hIndex; col >= 0; col--) {
        if (_letters.at(vIndex).at(col) == ' ') break;
        else if (!_highlights.at(vIndex).at(col)) {
            successOnLine = false; break;
        }
    }
    if (successOnLine) {
        for (int col = hIndex; col >= 0; col--) {
    ...
    return successOnCol || successOnLine;
}
